...
class ServicesApiTests(APITestCase):
	...
	def setUpTestData(cls):
		...
		cls.competence = Competence.objects.create(nom="Installation domestique")
		...
		cls.profile_a = ProfilPrestataire.objects.create(user=cls.provider_a)
		cls.profile_b = ProfilPrestataire.objects.create(user=cls.provider_b)

	def offer_url(self, offer):
		return reverse("prestataire-service-detail", args=[offer.pk])
	...
	def test_admin_can_create_category(self):
		...
		response = self.client.post(
			...
			{"nom": "Plomberie", "description": "Services plomberie", "statut": "ACTIVE"},
			format="json",
		)

		self.assertEqual(response.status_code, status.HTTP_201_CREATED)
		self.assertTrue(Categorie.objects.filter(nom="Plomberie").exists())
	...
	def test_public_catalog_path_returns_category_service_and_offers(self):
		...
		category_response = self.client.get(reverse("categorie-detail", args=[self.categorie.pk]))
		service_response = self.client.get(reverse("service-detail", args=[self.service.pk]))
		offers_response = self.client.get(
			reverse("service-prestataires", args=[self.service.pk])
		)
		...
		self.assertEqual(offers_response.data[0]["id"], str(offer.pk))
	...
	def test_provider_can_create_own_offer(self):
		...
		response = self.client.post(
			reverse("prestataire-service-list"),
			{
				...
				"competences": [str(self.competence.id)],
				"prix": "15000.00",
				"unite": "forfait",
		...
		self.assertEqual(response.status_code, status.HTTP_201_CREATED)
		offer = PrestataireService.objects.get(pk=response.data["id"])
		self.assertEqual(offer.prestataire, self.profile_a)
	...
	def test_provider_can_update_own_offer(self):
		...
		self.assertEqual(offer.prix, Decimal("17500.00"))
	...
	def test_provider_can_delete_own_offer(self):
		...
		response = self.client.delete(self.offer_url(offer))

		self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
		self.assertFalse(PrestataireService.objects.filter(pk=offer.pk).exists())
	...
	def test_anonymous_user_cannot_create_protected_resources(self):
		...
		self.assertEqual(offer_response.status_code, status.HTTP_401_UNAUTHORIZED)
